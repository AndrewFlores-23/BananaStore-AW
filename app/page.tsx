  ...
  return (
    <article className="product-card product-card-action" onClick={open} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); open(); } }} tabIndex={0} role="button" aria-label={`Ver detalles de ${product.name}`}>
      <div className="product-image">
        ...
        <button className={favorite ? 'is-favorite' : ''} onClick={(event) => { event.stopPropagation(); onFavorite(); }} aria-label={`${favorite ? 'Quitar' : 'Guardar'} ${product.name}`} aria-pressed={favorite}><Heart size={20} strokeWidth={2} fill={favorite ? 'currentColor' : 'none'} /></button>
        <span className="quick-view">Ver detalles <b><ArrowUpRight size={16} /></b></span>
      </div>
  ...
  return (
    <div className="product-modal-backdrop" onMouseDown={onClose} role="presentation">
      <section className="product-modal glass" role="dialog" aria-modal="true" aria-labelledby="product-modal-title" onMouseDown={(event) => event.stopPropagation()}>
        <button className="modal-close" onClick={onClose} aria-label="Cerrar detalle"><X size={21} /></button>
        ...
        <div className="modal-details">
          ...
          <a className="whatsapp-button" href={`https://wa.me/50687709970?text=${encodeURIComponent(message)}`} target="_blank" rel="noreferrer"><span className="whatsapp-icon"><MessageCircle size={18} /></span> Consultá disponibilidad por WhatsApp <b><ArrowUpRight size={17} /></b></a>
  ...
  return (
    <main>
      <nav className="glass nav-shell">
        ...
        <div className="nav-tools"><button className="icon-button" onClick={() => setSearchOpen(!searchOpen)} aria-label="Buscar"><Search size={20} /></button><a className="favorite-counter" href="#catalogo" aria-label={`${favorites.length} favoritos`}><Heart size={19} /> <span>{favorites.length}</span></a><a className="nav-cta" href="#catalogo">Ver colección <ArrowUpRight size={16} /></a></div>
        {searchOpen && <div className="glass search-popover"><label htmlFor="search">Buscar en el catálogo</label><input autoFocus id="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Camisa, mujer, vestido…" /><button onClick={() => { setQuery(''); setSearchOpen(false); }} aria-label="Cerrar búsqueda"><X size={19} /></button></div>}
      </nav>

      <section className="hero" id="inicio">
        <div className="hero-orb orb-one" /><div className="hero-orb orb-two" />
        <div className="hero-copy"><p className="eyebrow"><span /> Nueva colección 2026</p><h1>Tu estilo.<br /><em>Sin complicaciones.</em></h1><p className="hero-lead">Prendas frescas, versátiles y con mucha actitud. Elegí tu favorita y armá el look a tu manera.</p><div className="hero-actions"><a className="button button-dark" href="#catalogo">Explorar catálogo <ArrowDown size={17} /></a><a className="button button-glass" href="#nuevos">Lo más nuevo</a></div><div className="hero-note"><span className="avatar-stack"><i /><i /><i /></span><p><strong>Nuevos looks cada semana</strong><br />Elegidos para vos, con amor local.</p></div></div>
        <div className="hero-visual"><div className="hero-image-wrap"><img src={`${basePath}/catalog/camisa-sol.png`} alt="Modelo con camisa amarilla y pantalón crema" /><div className="glass look-card"><span className="look-dot" /><div><strong>Look del día</strong><small>Camisa Sol · ₡18.900</small></div><button onClick={() => setSelectedProduct(products[0])} aria-label="Ver Camisa Sol"><Plus size={19} /></button></div></div><div className="logo-badge glass"><img src={`${basePath}/banana-logo.jpeg`} alt="Logo de Banana Store" /></div><span className="float-label glass">Fresco · Local · Vos</span></div>
      </section>
      ...
      <section className="section section-preview" id="nuevos">
        <div className="section-heading"><div><p className="eyebrow"><span /> Recién llegados</p><h2>Lo nuevo está <em>aquí.</em></h2></div><button onClick={() => { setActiveFilter('Nuevos'); document.querySelector('#catalogo')?.scrollIntoView(); }}>Ver toda la colección <ArrowRight size={18} /></button></div>
