    ...
    <div className="product-modal-backdrop" onMouseDown={onClose} role="presentation">
      <section className="product-modal glass" role="dialog" aria-modal="true" aria-labelledby="product-modal-title" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="modal-details">
          ...
          <a className="whatsapp-button" href={`https://api.whatsapp.com/send?text=${encodeURIComponent(message)}`} target="_blank" rel="noreferrer"><span className="whatsapp-icon">◉</span> Consultá disponibilidad por WhatsApp <b>↗</b></a>
...
export default function Home() {
  ...
  return (
    <main>
      ...
      <section className="visit-section" id="visitanos"><div className="visit-card"><p className="eyebrow light"><span /> Vení a conocernos</p><h2>Tu próximo look<br /><em>te está esperando.</em></h2><div className="visit-grid"><div><span>Ubicación</span><strong>Consultá la dirección<br />por nuestras redes</strong></div><div><span>Horario demo</span><strong>Lun — Sáb<br />10:00 — 19:00</strong></div><div><span>Contacto demo</span><strong>@bananastore<br />+506 0000-0000</strong></div></div><p className="demo-note">Los datos de contacto son demostrativos y se pueden reemplazar por la información real del local.</p></div><div className="newsletter glass"><span>🍌</span><div><p className="eyebrow"><span /> Club Banana</p><h3>Enterate primero.</h3><p>Recibí novedades, nuevos ingresos y looks de la semana.</p></div><form onSubmit={(event) => event.preventDefault()}><label className="sr-only" htmlFor="email">Correo electrónico</label><input id="email" type="email" placeholder="tu@email.com" required /><button type="submit" aria-label="Suscribirse">→</button></form></div></section>
